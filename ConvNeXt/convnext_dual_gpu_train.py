# filepath: c:\Users\User\OneDrive - National ChengChi University\113-2 Design\project\ConvNeXt\convnext_dual_gpu_train.py
'''
ConvNeXt 圖像辨識程式碼 (PyTorch, 雙GPU, TensorBoard)
...
model_name = "convnext_base"  # 可以選擇 "convnext_tiny", "convnext_small", "convnext_base", "convnext_large"
num_epochs = 50
batch_size = 64  # 雙GPU可以考慮增加batch size
learning_rate = 0.001

# TensorBoard 設定
log_dir = 'runs/convnext_dual_gpu_experiment_{}'.format(time.strftime("%Y%m%d-%H%M%S"))
...
# --- ConvNeXt 模型定義 ---
def get_convnext_model(model_name, num_classes, pretrained=True):
    """
    取得 ConvNeXt 模型並修改最後一層以符合類別數量
    """
    model = None
    ...
    if model_name == "convnext_tiny":
        ...
        raise ValueError(f"ConvNeXt model {model_name} not recognized. Supported models: convnext_tiny, convnext_small, convnext_base, convnext_large")
    
    # 修改分類器層 - ConvNeXt 的分類器結構
    # ConvNeXt 的 classifier 包含多個層，我們需要修改最後的 Linear 層
    if hasattr(model.classifier, '2'):  # 檢查是否有第三層（索引2）
...
model = get_convnext_model(model_name, num_classes)
...
# --- 損失函數和優化器 ---
criterion = nn.CrossEntropyLoss(label_smoothing=0.1)  # ConvNeXt 受益於 label smoothing

# 對於ConvNeXt，使用AdamW優化器配合較強的權重衰減
optimizer = optim.AdamW(model.parameters(), lr=learning_rate, weight_decay=0.05)

# 學習率調整器 - 使用CosineAnnealingWarmRestarts對ConvNeXt效果較好
...
if __name__ == '__main__':
    if not os.path.exists(data_dir) or not os.listdir(data_dir):
        ...
        print("Starting ConvNeXt training with enhanced configuration...")
