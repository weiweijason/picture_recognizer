# filepath: c:\Users\User\OneDrive - National ChengChi University\113-2 Design\project\DenseNet\densenet_dual_gpu_train.py
'''
DenseNet 圖像辨識程式碼 (PyTorch, 雙GPU, TensorBoard)
...
model_name = "densenet201"  # 可以選擇 "densenet121", "densenet161", "densenet169", "densenet201"
num_epochs = 50
batch_size = 64  # 雙GPU可以考慮增加batch size
learning_rate = 0.001

# TensorBoard 設定
log_dir = 'runs/densenet_dual_gpu_experiment_{}'.format(time.strftime("%Y%m%d-%H%M%S"))
...
data_transforms = {
    'train': transforms.Compose([
        ...
        transforms.RandomVerticalFlip(p=0.1),  # DenseNet 可以受益於更多樣的增強
        transforms.ToTensor(),
...
# --- DenseNet 模型定義 ---
def get_densenet_model(model_name, num_classes, pretrained=True):
    """
    取得 DenseNet 模型並修改最後一層以符合類別數量
    """
    model = None
    ...
    if model_name == "densenet121":
        ...
        raise ValueError(f"DenseNet model {model_name} not recognized. Supported models: densenet121, densenet161, densenet169, densenet201")
    
    # 修改分類器層 - DenseNet 的分類器是單一 Linear 層
...
model = get_densenet_model(model_name, num_classes)
...
# 對於DenseNet，使用AdamW優化器配合權重衰減
optimizer = optim.AdamW(model.parameters(), lr=learning_rate, weight_decay=0.01)

# 學習率調整器 - 使用ReduceLROnPlateau對DenseNet效果較好
scheduler = optim.lr_scheduler.ReduceLROnPlateau(
    optimizer, 
    mode='max',  # 監控驗證準確率的最大值
    factor=0.5,  # 學習率衰減因子
    patience=5,  # 容忍epoch數
    verbose=True,
    min_lr=1e-7
...
def train_model(model, criterion, optimizer, scheduler=None, num_epochs=25):
    ...
    for epoch in range(num_epochs):
        ...
        # 學習率調整 - 使用驗證準確率進行調整
        if scheduler:
            scheduler.step(epoch_acc)  # ReduceLROnPlateau需要監控的指標
...
if __name__ == '__main__':
    if not os.path.exists(data_dir) or not os.listdir(data_dir):
        ...
        print("Starting DenseNet training with enhanced configuration...")
